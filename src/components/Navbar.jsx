...
const Navbar = () => {
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 h-20 flex items-center justify-between">
          ...
          <div className="flex-1 flex justify-start">
            <a href="#hero" className="nav-logo text-2xl md:text-4xl font-black tracking-tight text-[#2b1200] transition-colors duration-300 group flex items-center">
              M<span className="text-[#CCFF00] transition-colors duration-300 group-hover:text-[#2b1200]">.</span>AYAN
            </a>
          </div>
          ...
          >
            <span className="block w-6 h-0.5 bg-[#2b1200]"></span>
            <span className="block w-6 h-0.5 bg-[#2b1200]"></span>
            <span className="block w-6 h-0.5 bg-[#2b1200]"></span>
          </button>
      ...
      {menuOpen && (
        <div className="fixed inset-0 bg-[#FAF9F5] z-50 flex flex-col items-center justify-center p-8">
          <button
            className="absolute top-4 right-4 text-3xl font-bold text-[#2b1200] cursor-pointer"
          ...
          <nav className="flex flex-col items-center gap-6">
            {navItems.map((item) => (
              <a
                key={item.number}
                href={item.href}
                className="text-3xl font-black uppercase text-[#2b1200] tracking-[0.12em]"
                onClick={() => setMenuOpen(false)}
              >
                {item.label}
              </a>
            ...
            <a
              href="#resume"
              className="mt-8 text-2xl font-black uppercase text-[#2b1200] tracking-[0.12em] border-2 border-[#2b1200] px-4 py-2"
              onClick={() => setMenuOpen(false)}
            >
              RESUME
            </a>
